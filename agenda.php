...
if (isset($_POST['hiddenSelectedDate']) && isset($_POST['selectedTime'])) {
    $datacorte = mysqli_real_escape_string($conn, $_POST['hiddenSelectedDate']);
    $horario = mysqli_real_escape_string($conn, $_POST['selectedTime']);

    // Verifica se o agendamento já existe para o usuário
    $query = "SELECT * FROM agendamentos WHERE user_id = ?";
    $stmt = $conn->prepare($query);
    ...
    if ($result->num_rows > 0) {
        ...
        $update_query = "UPDATE agendamentos SET data_corte = ?, horario = ? WHERE user_id = ?";
        $update_stmt = $conn->prepare($update_query);
        $update_stmt->bind_param('ssi', $datacorte, $horario, $user_id);
        if ($update_stmt->execute()) {
            header("Location: /carrinho?datacorte=$datacorte&horario=$horario");
            exit();
        } else {
            echo "Erro ao atualizar agendamento: " . $update_stmt->error;
        }
        $update_stmt->close();
    } else {
        // Insere um novo agendamento
        $insert_query = "INSERT INTO agendamentos (user_id, data_corte, horario) VALUES (?, ?, ?)";
        $insert_stmt = $conn->prepare($insert_query);
        $insert_stmt->bind_param('iss', $user_id, $datacorte, $horario);
        if ($insert_stmt->execute()) {
            header("Location: /carrinho?datacorte=$datacorte&horario=$horario");
            exit();
        } else {
            echo "Erro ao inserir agendamento: " . $insert_stmt->error;
        }
        $insert_stmt->close();
    }
...
<div class="menu w3-bottombar reveal" style="z-index:1;">
    <strong><a href="/home"><i class="fa fa-home" style="font-size:25px"></i></a></strong>
    <p class="roboto-medium"><a class="itensMenu" href="/produtos"><strong>Cortes</strong></a></p>
    <p class="roboto-medium"><a class="itensMenu" href="/pacotes"><strong>Pacotes</strong></a></p>
    <p class="roboto-medium"><a class="itensMenu" href="/agenda"><strong>Agenda</strong></a></p>
    <div style="margin-left: auto;" class="icones-direita">
    <a href="/carrinho" class="carrinho"style="margin-right: 15px;"><i class="fa fa-shopping-cart " style="font-size:25px;"></i>  <span class="cart-count"><?php echo $cartItemCount; ?></span></a>
        <div class="user-menu">
            ...
            <div class="dropdown-content">
                ...
                <a href="/logout" style="font-size:12px">Sair <i class="fa fa-sign-out" aria-hidden="true"></i></a>
            </div>
        </div>
    </div>
</div>
...
<div class="w3-text-white w3-center reveal" style="margin-bottom:150px">
    ...
    <div class="reveal">
        <h1 class="w3-text-white w3-center" style="font-size:30px; font-family: fonteCadastro;">
            AGENDE SEU DIA: 
            <hr class="sublinhado">
        </h1>
    </div>
    ...
    <!-- CALENDÁRIO -->
    <div class="dark calendar reveal">
        <div class="calendar-header">
            <div class="year-picker">
                <span class="year-change" id="prev-month"><pre><</pre></span>
                <span class="month-picker" id="month-picker"></span>
            </div>
            <span id="year"></span>
            <span class="year-change" id="next-month"><pre>></pre></span>
        </div>
...
<form id="scheduleForm" action="/agenda" method="post">

<h4 class="w3-text-white reveal" style="font-family: fonteCadastro;">DIA:
    <input id="selectedDate" name="selectedDate" class="w3-border w3-round-xlarge" style="width:400px;background-color:#24282f; color: white;" disabled required readonly>
</h4>
<br><br><br>

<!-- HORÁRIOS -->
<h3 class="w3-text-white reveal" style="font-family: fonteCadastro;">SELECIONE UM HORÁRIO:
    <hr class="sublinhado">
</h3>
<br>
<div id="horarios" class="reveal horarios">
    <img class="w3-right" style="color:blue;width:15%;margin-right:10%;" src="images/pole.png" alt="">
    <img class="w3-left" style="color:blue;width:15%;margin-left:10%;" src="images/pole.png" alt="">
    <?php
    $horarios = [
    ...
    foreach ($horarios as $horario) {
        echo "<label class='time-option w3-button w3-round-large w3-margin'>
                <h4 style='margin:auto'><strong>$horario</strong></h4>
             <input type='radio' name='selectedTime' value='$horario' style='display:none;' required>
            </label>";
    }
    ?>
    <input type="hidden" id="hiddenSelectedDate" name="hiddenSelectedDate" value="">
    <br><br>
    <button type="submit" class="w3-button w3-round-large w3-margin" style="background-color:#1E90FF;color:white">Confirmar</button>
</div>
</form>
    </div>

<!-- Script para atualizar dinamicamente a data e horário na página -->
<script>
// Quando o formulário for enviado
document.getElementById('scheduleForm').addEventListener('submit', function(e) {
    // Remova o preventDefault para permitir o envio normal do formulário
    // e.preventDefault();

    // Capturar a data e horário selecionados
    const selectedDate = document.getElementById('selectedDate').value;
    const selectedTime = document.querySelector('input[name="selectedTime"]:checked').value;

    // Atualizar os campos da tela de carrinho
    document.getElementById('selectedDateDisplay').innerText = selectedDate ? selectedDate : "N/A";
    document.getElementById('selectedTimeDisplay').innerText = selectedTime ? selectedTime : "N/A";

    // O formulário agora será enviado normalmente, garantindo que as mudanças sejam salvas no banco de dados
});
...
document.getElementById('scheduleForm').addEventListener('submit', function(e) {
    ...
    if (!selectedDate || !selectedTime) {
        ...
        e.preventDefault(); // Evita o envio do formulário se não estiver preenchido
        return;
    }
    ...
    option.addEventListener('click', function() {
        // Remover a classe 'selected-time' de todos os botões
        document.querySelectorAll('.time-option').forEach(btn => btn.classList.remove('selected-time'));
        
        // Adicionar a classe 'selected-time' ao botão clicado
