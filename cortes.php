...
include_once 'processar_corte.php';
// Consulta para pegar cortes de cabelo e barba
$sql_cabelo = "SELECT nome_cabelo, preco_cabelo, imagem_cabelo FROM cabelo";
$sql_barba = "SELECT nome_barba, preco_barba, imagem_barba FROM barba";

$result_cabelo = $conn->query($sql_cabelo);
$result_barba = $conn->query($sql_barba);
session_start();
...
.card img {
    width: 90%; /* Reduz a largura para dar espaço à borda */
    height: auto;
    border-radius: 8px;
...
<div class="w3-text-white w3-center reveal" style="margin-bottom:150px">
      ...
      <div class="w3-center reveal cabelo"><h2 class="w3-text-white playfair-display-sc-bold" style="font-size:30px;">
            <strong>Cabelo</strong>
            <hr class="sublinhado">
            </h2>
        </div>
    <!--CABELO-->
    <div class="main w3-text-white reveal">
    <?php
        include('conBD.php'); // Inclua a conexão com o banco de dados

        // Consulta para buscar os produtos da tabela cabelo
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($cabelo = $result->fetch_assoc()) {
                echo '
                <div class="package card card2 w3-card-4 reveal" data-id="' . $cabelo['id'] . '" data-name="' . $cabelo['nome_cabelo'] . '" " data-img="' . $cabelo['imagem_cabelo'] . '" data-price="' . $cabelo['preco_cabelo'] . '">
                    <img class="w3-padding" src="images/'.$cabelo['imagem_cabelo'].'">
                    <h3 class="w3-center"><strong>' . $cabelo['nome_cabelo'] .' - R$'.number_format($cabelo['preco_cabelo'], 2, ',', '.'). '</strong></h3>
                    <form method="POST" action="add_to_cart.php">
                        <input type="hidden" name="product_id" value="' . $cabelo['id'] . '">
                        <input type="hidden" name="product_img" value="' . $cabelo['imagem_cabelo'] . '">
                        <input type="hidden" name="product_name" value="' . $cabelo['nome_cabelo'] . '">
                        <input type="hidden" name="product_price" value="' . $cabelo['preco_cabelo'] . '">
                        <button href="/agenda" type="submit" class="w3-button w3-center w3-margin-bottom w3-display-bottommiddle w3-blue w3-round-large add-to-cart">Agendar</button>
                    </form>
                </div>
                ';
            }
    ...
    <br>
    <br>
    <!-- Exibindo Cortes de Barba -->
            <div class="w3-center reveal outro"><h2 class="w3-text-white playfair-display-sc-bold" style="font-size:30px;">
...
// Consulta para buscar os produtos da tabela barba
$query = "SELECT * FROM barba"; 
$result = $conn->query($query);

// Verifica se encontrou produtos
if ($result && $result->num_rows > 0) {
    // Loop para exibir os produtos
    while ($barba = $result->fetch_assoc()) {
        echo '
        <div class="package card card2 w3-card-4 reveal" data-id="' . $barba['id'] . '" data-name="' . $barba['nome_barba'] . '" " data-img="' . $barba['imagem_barba'] . '" data-price="' . $barba['preco_barba'] . '">
            <img class="w3-padding" src="images/'.$barba['imagem_barba'].'">
            <h3 class="w3-center"><strong>' . $barba['nome_barba'] .' - R$'.number_format($barba['preco_barba'], 2, ',', '.'). '</strong></h3>
            <form method="POST" action="add_to_cart.php">
                <input type="hidden" name="product_id" value="' . $barba['id'] . '">
                <input type="hidden" name="product_img" value="' . $barba['imagem_barba'] . '">
                <input type="hidden" name="product_name" value="' . $barba['nome_barba'] . '">
                <input type="hidden" name="product_price" value="' . $barba['preco_barba'] . '">
                <button href="/agenda" type="submit" class="w3-button w3-center w3-margin-bottom w3-display-bottommiddle w3-blue w3-round-large add-to-cart">Agendar</button>
            </form>
        </div>';
    }
